...
import { useLikePostMutation, useCommentOnPostMutation, useRetweetPostMutation } from "../features/post/postApiSlice";
...
const Post = ({ post, token }) => {
    ...
    return (<Card raised={true} className="w-full h-44 overflow-scroll rounded-lg p-2 text-center shadow-4xl rounded-[18px]  ounded-lg bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-40% to-emerald-500 to-100%  dark:from-blue-700 dark:from-20% dark:via-emarald-700 dark:via-30% dark:to-ryan-700 dark:to-100% dark:text-amber-500 text-amber-800  flex flex-col justify-evenly items-center">
      ...
      {post.post.image ? (<div className="bg">
        ...
        </div>) : (<Paper className="flex justify-center m-0 items-center bg h-28 p-2">{post.post.content || ""}</Paper>)}
