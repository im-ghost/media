import { Typography, Container, Card, Button, CardActions, CardContent, CardMedia, Paper, Box, Tabs, Tab, AppBar } from "@mui/material";
...
const Profile = () => {
    ...
    if (!user) {
        return (<Card onClick={() => navigate("/login")}>
    <Typography variant="h1"> Login </Typography>
    </Card>);
    }
    return (<Container className="w-screen h-screen py-2 rounded flex items-center justify-center">
  {user && (<div className="w-[98vw] h-[98vh] p-2 rounded flex items-center justify-center">
    {user !== null && (<Container className="w-[100vw] h-[100vh]">
       <Card className="w-full">
     <CardMedia image={user.image ? user.image : Default} sx={{
                    height: 0,
                    paddingTop: '56.25%',
                    marginTop: '30'
                }}/>
    ...
    {user.posts && user.token && (<Box className="w-full h-auto min-h-64 border">
      ...
      <SwipeableViews axis={theme.direction === 'rtl' ? 'x-reverse' : 'x'} className="h-auto min-h-64 " index={value} onChangeIndex={handleChangeIndex}>
